...
beforeEach(function() {
  app = express();
  router = createRouter(app); // Express app router
  nodeRouter = createRouter(); // Generic router
});

describe("createRouter(app:express[, mapActionToHandler:function]):Router", function() {
  describe("Eager routing on Express App Router instance", function() {
  ...
  describe("Lazy routing on generic Router instance", function() {
    describe("router.route(options:object|string, action:*):void", function() {
      describe("basic functionality", function() {
        const tests = [
          {
        ...
        tests.forEach(test => {
          describe(test.description, function() {
            test.expected.forEach(expected => {
              it(`returns a ${expected.status} for the request ${expected.method} ${expected.url}`, function(done) {
                nodeRouter.route(test.options, test.action);

                nodeRouter.apply(function(route) {
      ...
      describe("different methods", function() {
        methods.forEach(method => {
          it(`successfully routes to ${method} requests`, function(done) {
            ...
            nodeRouter.route({ method, uri }, (req, res) => {
              res.send("OK");
      ...
      describe("middleware", function() {
        it("accepts an array of middleware", function(done) {
          ...
          nodeRouter.route({ middleware }, (req, res) => {
            res.send(req.cumulative[0]);
          });
      ...
      describe("named routes", function() {
        it("allows routes to be named, so that urls can be created later by referencing these names", function(done) {
          ...
          nodeRouter.route({ uri, name }, (req, res) => {
            res.send(nodeRouter.url(name));
          }).apply(function(route) {
            const { method, path, handlers } = route;
          ...
          supertest(app).get(uri).expect(200, nodeRouter.url(name), done);
        });
      });
    });
    ...
    describe("router.group(groupOptions:object, closure:function)", function() {
      it("creates route groups that can share common uri prefixes, middleware, namespaces, and patterns", function(done) {
        ...
        const getUserUrl = nodeRouter.url("base.version.users.getUser", {version: 1, user: 2});
        const getUserUrlResult = "/api/v1/users/2";
        ...
        const getNameUrl = nodeRouter.url("base.version.users.getName", {version: 1, user: "shaunpersad"});
        const getNameUrlResult = "/api/v1/users/shaunpersad/name";
    ...
    describe("router.url(name:string, params:object)", function() {
      it("generates urls to routes that have been named", function() {
        ...
        nodeRouter.route({ uri: "/", name: "index" }, (req, res) => {
          res.send("OK");
        });

        const barBaz = nodeRouter.url("bar.baz");
        const barBazResult = "/foo";
        ...
        const index = nodeRouter.url("index");
        const indexResult = "/";
  ...
  describe("mapActionToHandler(action:*, routeDescription:object, routeOptions:object)", function() {
    it("is called for each Express app route defined, and must return a request handler", function(done) {
      // eslint-disable-next-line
    ...
    it("is called for each generic Node app route defined, and must return a request handler", function(done) {
      // eslint-disable-next-line
      const router = createRouter((action, routeDescription, routeOptions) => {
        return action.customHandler;
      });
